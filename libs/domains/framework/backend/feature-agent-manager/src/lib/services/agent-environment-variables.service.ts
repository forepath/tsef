...
export class AgentEnvironmentVariablesService {
  ...
  async reconcileEnvironmentVariables(agentId: string): Promise<void> {
    try {
      ...
      // Update the container's environment
      await this.dockerService.updateContainer(agent.containerId, { env });

      this.logger.log(
        `Reconciled ${environmentVariables.length} environment variables for agent ${agentId} (container ${agent.containerId})`,
      );
    } catch (error: unknown) {
