...
import { BidirectionalChatFilter } from './bidirectional-chat-filter';

describe('BidirectionalChatFilter', () => {
  let filter: BidirectionalChatFilter;

  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [BidirectionalChatFilter],
    }).compile();

    filter = module.get<BidirectionalChatFilter>(BidirectionalChatFilter);
  });

  describe('getType', () => {
    it('should return "bidirectional-example"', () => {
      expect(filter.getType()).toBe('bidirectional-example');
    });
  });

  describe('getDisplayName', () => {
    it('should return "Bidirectional Filter Example"', () => {
      expect(filter.getDisplayName()).toBe('Bidirectional Filter Example');
    });
  });
  ...
  describe('filter', () => {
    ...
    it('should work with context parameter for user messages', async () => {
    ...
    it('should work with context parameter for agent messages', async () => {
      const result = await filter.filter('test-filter message', {
        agentId: 'test-agent-id',
        actor: 'agent',
      });
