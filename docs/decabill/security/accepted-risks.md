...
| **Configuration**                                 | **`PermitRootLogin yes`** and **root** `authorized_keys` installed via provisioning scripts (`libs/domains/decabill/backend/feature-billing-manager/src/lib/contributors/agenstra-controller/agent-controller.utils.ts`, `agenstra-manager/agent-manager.utils.ts`) |
