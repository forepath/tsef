...
- **Plan meters** attach via `GET/POST /service-plans/{id}/meters` (optional unit-price and included-usage overrides). Embedded on `ServicePlanResponse.meters` as the effective set: explicit plan attachments plus meters inherited from the plan’s service type (`inherited=true`). Inherited meters are fixed in the plan UI.
