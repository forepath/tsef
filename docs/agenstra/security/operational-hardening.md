...
| Practice                      | Detail                                                                                                                                                                                                                                                                                          |
| ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Non-root runtime**          | Debian-based backends and worker/VNC/SSH images run as **`agenstra`** (UID/GID **10001** by default); Alpine frontend servers run as **`node`** (**1000**).                                                                                                                                     |
...
| **Workspace mount ownership** | Worker, VNC, and SSH entrypoints `chown` bind-mounted agent data to `agenstra` when the host directory is root-owned.                                                                                                                                                                           |
...
Deploy **manager API, worker, VNC, and SSH images from the same release** when paths or users change. See **[Docker deployment](../deployment/docker-deployment.md)** and **[Backend Agent Manager](../applications/backend-agent-manager.md)**.
