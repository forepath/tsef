...
| Production      | 1    | 1 to 2 GiB | 5 GiB | Completed jobs retained for Bull Board history |
...
| Production      | 1 to 2 | 2 to 4 GiB   | Scale for concurrent console users and proxy traffic |
...
| Production      | 2 to 4 | 4 to 8 GiB   | Embedding and import batches are CPU/memory heavy |
...
| Profile          | vCPU        | Memory limit     | Notes                    |
| ---------------- | ----------- | ---------------- | ------------------------ |
| All environments | 0.25 to 0.5 | 512 MiB to 1 GiB | Singleton per deployment |
...
| Profile         | vCPU | Memory     | Disk   | Notes                                     |
| --------------- | ---- | ---------- | ------ | ----------------------------------------- |
| Local / staging | 1    | 1 to 2 GiB | 15 GiB |                                           |
| Production      | 2    | 2 to 4 GiB | 50 GiB | Grows with agents and deployment run logs |
...
| Profile         | vCPU   | Memory limit | Notes                                          |
| --------------- | ------ | ------------ | ---------------------------------------------- |
| Local / staging | 1      | 1 to 2 GiB   |                                                |
| Production      | 1 to 2 | 2 GiB        | Size for Docker API churn, not agent workloads |
...
| ------------------------- | ---------------- | ------------------------ | ------------------------------------------------- | ------------------------------------------------------------------------------ |
| `agenstra-manager-worker` | 2                | 2 to 4 GiB               | 10 to 50 GiB workspace under `/opt/agents/{uuid}` | Includes cursor-agent, OpenCode, Nx, Git; builds and `npm install` spike usage |
| `agenstra-manager-vnc`    | 2                | 2 to 4 GiB               | 5 GiB                                             | XFCE4 + Chromium at default **1920×1080**                                      |
| `agenstra-manager-ssh`    | 0.25             | 256 to 512 MiB           | -                                                 | SSH sidecar                                                                    |
| `agenstra-manager-agi`    | 1 to 2           | 1 to 2 GiB               | 2 GiB                                             | OpenClaw gateway on **18789**                                                  |

Idle agent workers still consume baseline memory; stop agents when not in use.

### Manager Host Totals (planning)

Add manager API, manager Postgres, and Docker overhead (~1 to 2 GiB) to per-agent totals.

| Agents (worker + VNC each) | Host vCPU | Host memory  | Host disk |
| -------------------------- | --------- | ------------ | --------- |
| 1                          | 4 to 6    | 12 to 16 GiB | 50 GiB    |
| 3                          | 8 to 12   | 24 to 32 GiB | 120 GiB   |
| 5                          | 12 to 16  | 32 to 48 GiB | 200 GiB   |
...
| ------------- | ------------------------- | ---- | ---------------- | ------------ |
| Agent console | `agenstra-console-server` | 0.5  | 512 MiB to 1 GiB | **4200**     |
...
| Two agent stacks       | 8            | 12 to 16 GiB     |
| **Host total**         | **10 to 12** | **16 to 20 GiB** |
