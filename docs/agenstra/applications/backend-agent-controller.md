...
- **`clients`** (default `WEBSOCKET_NAMESPACE` = `clients`) to proxy to the selected workspace’s agent-manager; chat, terminals, stats, and controller-originated ticket hints for the chat UI
- **`tickets`** (default `TICKETS_WEBSOCKET_NAMESPACE` = `tickets`) to ticket board and automation realtime (see [Tickets and Workspaces](../features/tickets-and-workspaces.md))
